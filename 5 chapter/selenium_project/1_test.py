...
from selenium_project_4 import LoginPage
...
class Shop:
    def select_product(self):
        ...
        for login_name in login_standard_user:
            try:
                login_page.authorisation(login_name=login_name, login_password=password_all)
                if login_page.is_login_successful():
                    print(f"Successfully logged in with {login_name}")
                    login_page.logout()
                else:
